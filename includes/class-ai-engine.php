<?php
/**
 * کلاس مدیریت اتصال به مدل‌های هوش مصنوعی (مدل gemini-1.5-flash)
 */

if ( ! defined( 'ABSPATH' ) ) {
    exit;
}

class WPSmartAI_Engine {

    /**
     * ارسال پرومپت به مدل جمینی گوگل و دریافت پاسخ متنی
     */
    public static function call_gemini( $prompt, $api_key = '' ) {
        if ( empty( $api_key ) ) {
            $settings = get_option( 'wp_smart_ai_seo_settings', array() );
            $api_key = isset( $settings['api_key'] ) ? $settings['api_key'] : '';
        }

        if ( empty( $api_key ) ) {
            return new WP_Error( 'missing_api_key', 'لطفا کلید API هوش مصنوعی جمینی را در بخش تنظیمات وارد کنید.' );
        }

        // استفاده از مدل فوق‌العاده سریع و استاندارد گوگل gemini-1.5-flash جهت جلوگیری از خطای ۴۰۰/۴۰۴ گوگل
        $url = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=' . rawurlencode( $api_key );

        $body = array(
            'contents' => array(
                array(
                    'parts' => array(
                        array(
                            'text' => $prompt
                        )
                    )
                )
            ),
            'generationConfig' => array(
                'temperature' => 0.7,
                'maxOutputTokens' => 8192
            )
        );

        $response = wp_remote_post( $url, array(
            'headers' => array(
                'Content-Type' => 'application/json'
            ),
            'body'    => wp_json_encode( $body ),
            'timeout' => 120
        ) );

        if ( is_wp_error( $response ) ) {
            return $response;
        }

        $code = wp_remote_retrieve_response_code( $response );
        $response_body = wp_remote_retrieve_body( $response );

        if ( $code !== 200 ) {
            return new WP_Error( 'api_error', 'خطا در ارتباط با سرور گوگل (ممکن است کلید API نادرست باشد): ' . $response_body );
        }

        $data = json_decode( $response_body, true );
        if ( isset( $data['candidates'][0]['content']['parts'][0]['text'] ) ) {
            return $data['candidates'][0]['content']['parts'][0]['text'];
        }

        return new WP_Error( 'invalid_response', 'پاسخ نامعتبر از API دریافت شد.' );
    }

    /**
     * ترجمه و تولید ۳ کلمه کلیدی پرومپت انگلیسی فوق حرفه‌ای مخصوص موتورهای تصویرساز هوش مصنوعی (FLUX / Stable Diffusion)
     */
    public static function translate_keyword_to_english( $keyword ) {
        $prompt = "تو یک مهندس پرومپت هوش مصنوعی تصویرساز هستی. کلمه کلیدی فارسی زیر مربوط به صنعت آسانسور، پله برقی و بالابر است:
'{$keyword}'

وظیفه تو تولید ۳ پرومپت انگلیسی خیره‌کننده، بسیار دقیق، کوتاه و توصیفی مخصوص مدل‌های تصویرساز هوش مصنوعی (مانند Stable Diffusion و FLUX) است.
پرومپت‌ها باید به گونه‌ای طراحی شوند که:
۱. کاملاً واقعی و فوتورئالیستی باشند (photorealistic, 8k resolution, cinematic lighting).
۲. در پایان هر پرومپت حتماً عبارات منفی جهت عدم درج هیچگونه متن یا نوشته روی تصویر قرار گیرد: 'realistic photo, professional architecture photography, no text, no words, no letters, no watermark'.
۳. تصاویر باید کاملاً مرتبط با قطعات آسانسور، دکوراسیون کابین، شفت یا لابی ساختمان باشند.

خروجی را دقیقاً در قالب فرمت JSON زیر برگردان و هیچ متن اضافه دیگری ننویس:
[
  \"پرومپت تصویر اول مخصوص هوش مصنوعی به انگلیسی\",
  \"پرومپت تصویر دوم مخصوص هوش مصنوعی به انگلیسی\",
  \"پرومپت تصویر سوم مخصوص هوش مصنوعی به انگلیسی\"
]";

        $response = self::call_gemini( $prompt );
        if ( is_wp_error( $response ) ) {
            return array(
                'photorealistic high resolution modern luxury glass elevator cabin, steel and glass interior, cinematic lighting, realistic architecture photography, no text, no words, no letters',
                'photorealistic professional close-up of elevator traction machine steel motor engine details, no text, no words, no letters',
                'photorealistic wide angle luxury building lobby with modern glass escalators and high ceilings, no text, no words, no letters'
            );
        }

        $clean_json = preg_replace( '/```json|```/', '', $response );
        $clean_json = trim( $clean_json );
        $data = json_decode( $clean_json, true );

        if ( is_array( $data ) && count( $data ) >= 3 ) {
            return array_map( 'sanitize_text_field', $data );
        }

        return array(
            'photorealistic high resolution modern luxury glass elevator cabin, steel and glass interior, cinematic lighting, realistic architecture photography, no text, no words, no letters',
            'photorealistic professional close-up of elevator traction machine steel motor engine details, no text, no words, no letters',
            'photorealistic wide angle luxury building lobby with modern glass escalators and high ceilings, no text, no words, no letters'
        );
    }

    /**
     * تولید نامک انگلیسی کوتاه و از نظر لغوی کاملاً درست برای کلمه کلیدی فارسی
     */
    public static function generate_english_slug( $keyword ) {
        $prompt = "تو یک سئوکار فوق حرفه‌ای هستی. برای کلمه کلیدی فارسی زیر، یک نامک (Slug) انگلیسی بسیار کوتاه (حداکثر ۳ یا ۴ کلمه)، معنایی، کاملاً انگلیسی و سئوشده تولید کن که کلمات با خط فاصله (-) از هم جدا شده باشند.
کلمه کلیدی فارسی: '{$keyword}'
خروجی فقط و فقط باید نامک انگلیسی نهایی باشد و هیچ توضیح اضافه دیگری ننویس:";

        $response = self::call_gemini( $prompt );
        if ( is_wp_error( $response ) ) {
            return sanitize_title( $keyword );
        }

        return trim( strtolower( sanitize_title( $response ) ) );
    }

    /**
     * ساخت پرومپت اختصاصی برای سئو فوق حرفه‌ای با رفع تمامی خطاهای Yoast / Rank Math و خروجی فوق‌العاده زیبای CSS مطابق تمپلیت ارسالی کاربر
     */
    public static function generate_optimized_content( $content, $keyword, $tone = 'friendly', $custom_prompt = '', $custom_links = '', $is_pillar = false ) {
        $tone_farsi = 'دوستانه و صمیمی';
        if ( $tone === 'formal' ) {
            $tone_farsi = 'رسمی و اداری';
        } elseif ( $tone === 'expert' ) {
            $tone_farsi = 'تخصصی و علمی';
        }

        // دستورالعمل اختیاری کاربر
        $user_instructions = '';
        if ( ! empty( $custom_prompt ) ) {
            $user_instructions = "دستورالعمل اختصاصی کاربر که حتماً باید دقیقاً اعمال شود:\n{$custom_prompt}\n";
        }

        // لیست لینک‌های دلخواه کاربر جهت درج در متن
        $links_instructions = '';
        if ( ! empty( $custom_links ) ) {
            $links_instructions = "دستورالعمل لینک‌دهی: حتماً به طور کاملاً طبیعی و متناسب با متن، از لینک‌های سفارشی زیر روی انکرتکست‌های مرتبط مقاله استفاده کن (علاوه بر لینک‌های پیش‌فرض):\n{$custom_links}\n";
        }

        $pillar_instructions = '';
        if ( $is_pillar ) {
            $pillar_instructions = "این مقاله یک خوشه (Cluster) از یک ساختار پیلار است. بنابراین لینک‌دهی داخلی باید کاملاً متمرکز به مقاله مادر اصلی باشد و به مقالات یا صفحات دیگر لینک‌دهی نکنید تا لینک جویس منتقل شود.";
        }

        // آدرس‌های ثابت و واقعی سایت جهت لینک‌دهی دقیق
        $call_link = 'https://keshavarzlift.ir/#call';
        $shop_link = 'https://keshavarzlift.ir/shop/';
        $articles_link = 'https://keshavarzlift.ir/%d9%85%d9%82%d8%a7%d9%84%d8%a7%d8%aa-%d9%88-%d8%af%d8%a7%d9%86%d8%b3%d8%aa%d9%86%db%8c-%d9%87%d8%a7/';
        $order_link = 'https://keshavarzlift.ir/%D9%81%D8%B1%D9%85%20%D8%B3%D9%81%D8%A7%D8%B1%D8%B4/';

        $prompt = "تو یک کارشناس فوق‌العاده ارشد سئو و طراح فرانت‌اند حرفه‌ای هستی. وظیفه تو نوشتن یک مقاله فوق‌العاده خیره‌کننده، سئوشده و جذاب درباره موضوع و کلمه کلیدی اصلی '{$keyword}' است.

قوانین سخت‌گیرانه سئو و نگارش که باید دقیقاً رعایت شوند (برای حل ارورهای Rank Math و Yoast):
۱. **طول متن (بسیار مهم):** طول متن مقاله حتماً باید طولانی و جامع (بین ۱۰۰۰ الی ۱۵۰۰ کلمه) باشد. هرگز مقاله کوتاه یا خلاصه ننویسید! برای پر کردن این طول مقاله، از بخش‌ها، زیرعنوان‌ها، تعاریف و توضیحات فنی غنی استفاده کنید.
۲. **طول جملات:** حداقل ۷۵ درصد جملات باید کوتاه باشند (کمتر از ۱۵ واژه). جملات طولانی را بشکن و با نقطه جدا کن تا ارورهای خوانایی Yoast و Rank Math حل شوند.
۳. **هدینگ‌ها:** هر ۱۵۰ الی ۲۰۰ کلمه باید یک تگ h2 یا h3 قرار گیرد.
۴. **کلمه کلیدی در زیرعنوان‌ها (بسیار مهم):** فقط در ۳۰ الی ۴۰ درصد از هدینگ‌ها (H2, H3) از کلمه کلیدی اصلی '{$keyword}' یا کلمات هم‌خانواده استفاده کن. استفاده بیش از حد از کلمه کلیدی در زیرعنوان‌ها جریمه دارد. بنابراین ۶۰ الی ۷۰ درصد هدینگ‌ها را بدون کلمه کلیدی بنویس.
۵. **چگالی کلمه کلیدی:** کلمه کلیدی تمرکزی '{$keyword}' را فقط و فقط بین ۳ تا ۵ بار در کل متن بگنجان (به شکل ضخیم <strong>'{$keyword}'</strong> در اولین پاراگراف، بدنه متن و پاراگراف آخر). هرگز بیش از ۵ بار تکرار نکن تا خطای Keyword Stuffing رخ ندهد!
۶. **لینک‌های داخلی دقیق و فعال:**
   در صورتی که مقاله پیلار نیست، از لینک‌های واقعی زیر استفاده کن و کلماتی مثل 'خرید'، 'فروشگاه'، 'فرم سفارش'، 'تماس' را دقیقاً به این آدرس‌ها لینک ده:
   - لینک تماس و مشاوره: {$call_link} (مثلاً برای بخش‌های مشاوره تلفنی)
   - لینک فروشگاه قطعات و آسانسور: {$shop_link}
   - لینک مقالات و دانستنی‌های آسانسور: {$articles_link}
   - لینک فرم سفارش طراحی و ساخت آسانسور: {$order_link}
   (توجه: لینک‌دهی باید بسیار طبیعی باشد و نباید زیاده‌روی شود، بلکه در جاهای کاملاً مرتبط استفاده شود.)
۷. **لینک‌های خارجی:** حداقل ۱ لینک خارجی طبیعی به منابع مرجع معتبر جهانی مرتبط با صنعت آسانسور (مثل wikipedia) بده.
۸. **حفظ تگ‌های تصویر و قرار دادن تصاویر جدید در صورت نبودن آنها:**
   - اگر در متن ورودی تگ‌های تصویر <img> یا <figure> وجود دارند، آن‌ها را دقیقاً در جاهای مناسب متن جدید حفظ کن و تغییر نده.
   - در صورتی که متن ورودی فاقد تصویر است، دقیقاً ۳ تگ کامنت تصویری برای جایگذاری هوشمند عکس در متن قرار بده (یکی در بالا برای هدر/شاخص، یکی در وسط، یکی در پایین). تگ‌ها باید دقیقاً به این ساختار باشند:
     <!-- PLACE_IMAGE: توصیف تصویر به انگلیسی بدون متن داخل عکس | متن آلت تصویر شامل کلمه کلیدی یا کلمات هم‌خانواده و مترادف '{$keyword}' به فارسی جهت بهینه‌سازی ویژگی alt تصاویر -->
     (مثال: <!-- PLACE_IMAGE: modern elevator glass cabin inside luxury building | انواع آسانسور شیشه ای لوکس مناسب خرید ساختمان -->)
     *توجه:* در توصیف انگلیسی تاکید کن عکس واقعی و بدون هیچ نوشته یا متنی روی تصویر باشد (no text on image, realistic).

۹. **الزام درج بخش سوالات متداول (FAQ) و جعبه تماس (CTA) تمام‌عرض بدون بک‌گراند کلی:**
   - روی کل محتوا یا تگ div اصلی هیچگونه بک‌گراند تیره یا روشن قرار نده (چون پس‌زمینه کل سایت شما مشکی است و نباید خراب شود). پس‌زمینه را کاملاً شفاف (transparent) بگذار.
   - در پایان مقاله، حتماً یک بخش بسیار شیک سوالات متداول (FAQ) با حداقل ۳ سوال و پاسخ کلیدی و جذاب به همراه تگ‌های h3 و پاسخ‌های خوانا قرار بده.
   - هر بار از یک دیزاین، رنگ‌بندی و استایل متفاوت، جذاب و خلاقانه (مانند افکت‌های نئونی، سایه‌های رنگی، کادرهای شیشه‌ای با rgba یا بوردرهای گرادینت) برای دکمه‌ها، FAQ و CTA استفاده کن تا تکراری نشود. بک‌گراند برای کادرها فقط به صورت rgba ملایم تیره مجاز است.
   - حتماً یک جعبه تماس (CTA) تمام‌عرض مدرن با هاله نور نئونی دور کادر (مانند افکت‌های نئونی آبی، سبز، بنفش یا نارنجی) بساز که شامل دکمه تماس تلفنی به شماره '09221267593' و لینک به فرم سفارش باشد.

{$user_instructions}
{$links_instructions}
{$pillar_instructions}

لحن مقاله باید کاملاً '{$tone_farsi}' باشد.

ساختار طراحی ظاهری HTML که باید دقیقاً کپی و شبیه‌سازی شود:
کل مقاله را داخل یک تگ div قرار بده و تمام بخش‌ها را با استایل‌های inline دقیقاً مانند الگوی زیر طراحی کن:
- کل ظرفیت کدهای HTML باید تمام‌عرض و ریسپانسیو باشد.
- تگ‌های h2 با استایل: style=\"color: #00c8ff; font-size: clamp(20px, 2.5vw, 26px); font-weight: 700; margin-top: 40px; border-right: 4px solid #00c8ff; padding-right: 14px;\"
- تگ‌های p با استایل: style=\"color: #d7e4f5; font-size: clamp(15px, 1.5vw, 17px); line-height: 2; margin-bottom: 20px;\"
- لیست‌ها (ul) با استایل: style=\"color: #d7e4f5; font-size: clamp(15px, 1.5vw, 17px); line-height: 2; list-style: none; padding: 0;\" به همراه ایموجی‌های تیک سبز (✅) در ابتدای liها.
- جدول استاندارد و کاملاً ریسپانسیو با استایل‌های دقیق rgba و تگ‌های th/td با سایه‌های ظریف و طراحی تیره مدرن.
- نقل قول یا بخش برجسته (blockquote/div) با استایل: style=\"border-right: 4px solid #00c8ff; padding: 16px 20px; margin: 30px 0; background: rgba(0,200,255,0.05); border-radius: 12px; color: #b0c4e0; font-style: italic;\"

متن قدیمی یا موضوع کلی جهت نگارش مقاله:
---
{$content}
---

خروجی باید فقط و فقط بدنه اصلی مقاله به صورت کدهای HTML تمیز و طراحی شده به زبان فارسی باشد. هیچ پیام, مقدمه، توضیح انگلیسی، تگ markdown یا کد ```html اضافی صادر نکن:";

        return self::call_gemini( $prompt );
    }
}
